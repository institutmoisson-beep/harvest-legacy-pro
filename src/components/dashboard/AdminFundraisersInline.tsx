...
import { Plus, Eye, Users, Trash2 } from 'lucide-react';
...
const FUNDRAISER_CATEGORIES = [
  ...
  { value: 'business', label: '💼 Projet' },
];

export default function AdminFundraisersInline() {
  ...
  const { data: contribs } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from('fundraiser_contributions')
        .select('*').eq('fundraiser_id', showContribs).order('created_at', { ascending: false });
      if (error) throw error;
      return data as any[];
    },
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        goal_amount: Number(form.goal_amount) || 0, end_date: form.end_date, category: form.category,
        status: form.status, is_public: form.is_public, payment_link: form.payment_link || null, created_by: user?.id,
      };
      if (editingId) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as any).from('fundraisers').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-fundraisers'] }); toast({ title: "Cagnotte supprimée" }); },
  });
  ...
  const totalCollected = fundraisers?.reduce((s: number, f: any) => s + (f.current_amount || 0), 0) || 0;
  const totalContributors = fundraisers?.reduce((s: number, f: any) => s + (f.contributors_count || 0), 0) || 0;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold flex items-center gap-2">💝 Gestion des Cagnottes</h2>
        <Button onClick={() => { setShowForm(true); setEditingId(null); setForm({ ...emptyForm }); }}>
          <Plus className="h-4 w-4 mr-1" /> Créer une cagnotte
        </Button>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card><CardContent className="p-4 text-center">
          <p className="text-2xl font-bold">{fundraisers?.length || 0}</p>
          <p className="text-xs text-muted-foreground">Total cagnottes</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-2xl font-bold">{fundraisers?.filter((f: any) => f.status === 'active').length || 0}</p>
          <p className="text-xs text-muted-foreground">Actives</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-2xl font-bold">{totalCollected.toLocaleString()}</p>
          <p className="text-xs text-muted-foreground">Collectés (FCFA)</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
            ...
            <Card key={f.id}>
              <CardContent className="p-4 space-y-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Button size="sm" variant="ghost" className="text-destructive" onClick={() => { if (confirm('Supprimer ?')) deleteMutation.mutate(f.id); }}>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div><Label>Lien de paiement (optionnel)</Label><Input value={form.payment_link} onChange={e => setForm({ ...form, payment_link: e.target.value })} placeholder="https://pay.example.com/..." /></div>
            <div>
              <Label>Statut</Label>
