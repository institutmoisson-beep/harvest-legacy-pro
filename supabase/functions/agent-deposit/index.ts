...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Agent deposit request:', { agentId: user.id, memberCode, amount });
    ...
    if (agentWalletError || !agentWallet) {
      throw new Error('Portefeuille agent introuvable');
    }

    if (agentWallet.balance < amount) {
      throw new Error('Solde insuffisant dans le portefeuille agent');
    }
    ...
    if (debitError) {
      ...
      throw new Error('Erreur lors du débit du portefeuille agent');
    }
    ...
    if (creditError) {
      ...
      // Rollback agent debit
      await supabase.rpc('increment_wallet_balance', {
        p_user_id: user.id,
        p_amount: amount
      });
      throw new Error('Erreur lors du crédit du portefeuille membre');
    }
