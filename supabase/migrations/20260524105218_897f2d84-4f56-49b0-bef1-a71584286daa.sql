...
BEGIN
  ...
  IF p_delivery_address IS NULL OR length(trim(p_delivery_address)) < 5 THEN
    RETURN QUERY SELECT false, 'Adresse de livraison requise'::TEXT, NULL::UUID; RETURN;
  END IF;
  IF p_delivery_phone IS NULL OR length(trim(p_delivery_phone)) < 6 THEN
    RETURN QUERY SELECT false, 'Téléphone de livraison requis'::TEXT, NULL::UUID; RETURN;
  END IF;
  ...
  VALUES (
    v_pack.id, v_user, v_pack.price, v_pack.benefit_amount, 'completed',
    p_delivery_address, p_delivery_city, p_delivery_phone, p_delivery_notes
  )
