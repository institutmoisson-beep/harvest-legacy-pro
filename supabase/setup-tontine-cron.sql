-- Configuration du cron job pour les rappels automatiques de tontine
-- À exécuter manuellement dans l'éditeur SQL Supabase

-- Créer le cron job qui s'exécute tous les jours à 8h du matin
SELECT cron.schedule(
  ...
  SELECT
    net.http_post(
        url:='https://swefwubntyyfqaerlwym.supabase.co/functions/v1/tontine-auto-reminder',
...
-- SELECT * FROM cron.job WHERE jobname = 'tontine-auto-reminder-daily';

-- Pour désactiver le cron job:
-- SELECT cron.unschedule('tontine-auto-reminder-daily');

-- Pour réactiver avec une nouvelle fréquence (exemple: toutes les 12 heures):
-- SELECT cron.schedule(
--   'tontine-auto-reminder-daily',
--   '0 */12 * * *',
--   $$...(même contenu)...$$
